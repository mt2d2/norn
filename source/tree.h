...
public:
  NumberExprAST(double v, bool is_float) : val(v) {
    type = is_float ? TypeFactory::get_instance().get(FLOAT)
                    : TypeFactory::get_instance().get(INT);
  }
...
public:
  StringExprAST(std::string s) : val(s) {
    type = TypeFactory::get_instance().get(CHAR_ARY);
  }
  virtual void emit_bytecode(BuildContext &out);
...
public:
  VariableFieldExprAST(const std::string &n, const std::string &f)
      : name(n), field(f) {}
  virtual void emit_bytecode(BuildContext &out);
...
public:
  VariableAssignExprAST(const std::string &name, ExprAST *value)
...
public:
  VariableAssignFieldExprAST(const std::string &name, const std::string field,
                             ExprAST *value)
...
public:
  VariableAssignShortExprAST(const std::string &name, ExprAST *value)
      : VariableAssignExprAST(name, value) {}
  virtual void emit_bytecode(BuildContext &out);
...
public:
  ArrayIndexAssignExprAST(std::string id, ExprAST *index, ExprAST *assignee)
      : name(id), index(index), value(assignee) {
...
public:
  ArrayIndexAccessExprAST(const std::string &id, ExprAST *index)
      : VariableExprAST(id), index(index) {}
  virtual void emit_bytecode(BuildContext &out);
...
public:
  BinaryExprAST(char o, ExprAST *lhs, ExprAST *rhs)
    ...
    if (left->type == TypeFactory::get_instance().get(FLOAT) ||
...
public:
  ...
  virtual ~CallExprAST() {
    for (std::vector<ExprAST *>::iterator i = Args.begin(); i != Args.end();
         ++i)
      delete *i;
  }
...
public:
  ...
  virtual ~IfExprAST() {
    delete cond;
    for (std::vector<ExprAST *>::iterator i = then.begin(); i != then.end();
         ++i)
      delete *i;
    for (std::vector<ExprAST *>::iterator i = otherwise.begin();
         i != otherwise.end(); ++i)
      delete *i;
  }
...
public:
  ...
  virtual ~ForExprAST() {
    ...
    delete End;
    delete Step;
    for (std::vector<ExprAST *>::iterator i = body.begin(); i != body.end();
         ++i)
      delete *i;
  }
...
public:
  ...
  virtual ~WhileExprAST() {
    delete cond;
    for (std::vector<ExprAST *>::const_iterator i = body.begin();
         i != body.end(); ++i)
      delete *i;
  }
...
class PrototypeAST {
public:
  PrototypeAST(const std::string &n,
               const std::map<std::string, std::string> &a,
               const std::string &ftype)
      : args(std::map<std::string, Type>()), name(n),
        type(TypeFactory::get_instance().get(ftype)) {
    for (std::map<std::string, std::string>::const_iterator i = a.begin();
         i != a.end(); ++i)
      args[i->first] = TypeFactory::get_instance().get(i->second);

    name = n;
    for (std::map<std::string, Type>::iterator i = args.begin();
         i != args.end(); ++i)
...
class StructAST {
public:
  StructAST(const std::string &name, std::map<std::string, std::string> members)
