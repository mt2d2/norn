...
Block *BuildContext::get_block() { return working_block; }
...
uint8_t BuildContext::get_and_increment_seed() { return this->seed++; }

#if !NOJIT
uint8_t BuildContext::get_and_increment_loop_count() {
  ...
  for (const auto &elem : Args) {
    if (CallExprAST *call_ast = dynamic_cast<CallExprAST *>(elem))
      (elem)->type = out.get_block_type(call_ast->callee_signature(out));
                 ...
                 dynamic_cast<ArrayIndexAccessExprAST *>(elem)) {
    ...
    else if (VariableFieldExprAST *field_ast =
                 dynamic_cast<VariableFieldExprAST *>(elem)) {
